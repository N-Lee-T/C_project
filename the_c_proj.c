 ...
 */
 char *str_gsub(char *restrict *restrict haystack, char const *restrict n1, char const *restrict sub)
  
  {
      ...
      // NT note: strstr just finds the first occurence of substring2 in substring1
...
// All other functions and code are from NT
...
int expand(char *lnptr){
  ...
  if (asprintf(&status, "%i", fgkidstat) == -1) {     
...
int main(void){
  ...
  sigaction(SIGINT, &sigint_do, NULL); 
  sigaction(SIGTSTP, &sigint_do, NULL); 
